...
## Remaining-page rollout (2026-09-23)

Josh approved continuing beyond the public-page checkpoint, reported successful authenticated Members navigation, and confirmed that a tester using system dark mode saw the correct theme. The shared theme controller remains unchanged.

Sign-in and `/donate/` now use the shared header, page heading, theme selector, skip link and footer. Sign-in presents its existing login/signup/recovery forms before the longer introduction, adds visible associated labels to all four credential fields, and uses the same control palette and focus treatment. The one reference to the form being “below” was corrected for its new position. Authentication inline scripts, field IDs, form/button types, autocomplete attributes, constraints and redirect behavior are preserved. Donation retains its exact PayPal destinations and content, using the public collection surfaces and shared buttons. The existing donation redirect and historical Wix archive stay intact.

The original footer logo now has a neutral light backing and modest padding at 120px width on every main route, keeping navy artwork visible in dark mode. No image assets were changed. Member tools receive small CSS-only refinements for remaining selects/textareas, game fields, photo/note cards, disclosure hit areas and a photo grid that fits narrow screens. No member logic, data, permissions or writes changed.

Validation: 68 tests across the existing 19 test files pass, as do snapshot/duplicate validation, the build and whitespace checks. Built sign-in/donation assets resolve (public config is generated by the build). Browser checks covered desktop Light/Dark and 320px layouts, visible keyboard focus, empty Login/Sign Up/Forgot Password feedback, hidden recovery form in anonymous mode, and the real anonymous Members redirect to sign-in. Donation URLs were inspected without initiating a payment. Footer contrast was visually checked in dark mode. Live password reset/sign-in submissions and privileged tool workflows were not exercised; authenticated baseline navigation is user-reported. No mock data, fake sessions or payment transactions were used.


## Header utility row (2026-09-23)

Appearance now lives beneath the primary navigation inside the shared header, above its divider, beside the logo on desktop. All nine main routes use the same structure. The homepage's separate toolbar is removed, and the Events welcome card is back in its original introductory layout; neither hero needs to accommodate the selector. Mobile navigation and utilities wrap without horizontal overflow at 320px.

Signed-in visitors now see a direct My Account link and a visible Sign out button beside Appearance. Signed-out visitors retain Members and no sign-out action. The shared controller still calls the existing auth service, redirects out of Members after logout, returns keyboard focus to the Members link on public pages, and leaves a visible retry message if logout fails. The existing sidebar logout remains available on Members. Theme preference behavior is unchanged.

Validation: 71 tests pass, including executable account-controller cases for auth changes, nested donation paths, successful logout, dashboard redirect, failed logout/retry and stale initial-session handling. Normal build, snapshot validation, duplicate-event checks and whitespace checks pass. Desktop Home/Events visuals and eight anonymous routes at 320px were checked; the header selector is consistent and no page overflow was observed. Live account sign-out was not exercised; session behavior is covered with isolated unit-test doubles only.
