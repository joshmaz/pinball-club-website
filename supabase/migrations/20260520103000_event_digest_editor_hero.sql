-- Event editor digest: show hero preview even when the event row is not published yet.
-- Public consumers still use snh_public_event_promo_asset (requires published event).
...
comment on function public.snh_event_photo_digest_for_editor(uuid) is
  'Albums with event_id = p_event_id plus event hero payload (editor preview; event may be draft).';
